...
//        	throw new ArtWorkAlreadyExist("There is already an Artwork present with the same id.");
        	return false;
        }
    }
	...
	public boolean updateArtwork(Artwork artwork) {
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
			...
			preparedStatement.setDate(3, java.sql.Date.valueOf(artwork.getCreationDate()));
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
			...
			try (ResultSet resultSet = preparedStatement.executeQuery()) {
				if (resultSet.next()) {
					...
					artwork.setCreationDate(resultSet.getDate("CreationDate").toLocalDate());
					artwork.setMedium(resultSet.getString("Medium"));
					artwork.setImageURL(resultSet.getString("ImageURL"));
					return artwork;
				} else {
					throw new ArtWorkNotFoundException("No artworkId is found...");
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public List<Artwork> getUserFavoriteArtworks(int userId) {
		...
		String query = "SELECT A.* FROM Artwork A " + "JOIN User_Favorite_Artwork UFA ON A.ArtworkID = UFA.ArtworkID "
				+ "WHERE UFA.UserID = ?";
		try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
			preparedStatement.setInt(1, userId);
