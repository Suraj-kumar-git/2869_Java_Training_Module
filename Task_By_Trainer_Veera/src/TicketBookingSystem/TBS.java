	        ...
	        if(seatBooking<totalSeats) {
	        	if(isAbove50) {
	        		if(remainFrom50-seatBooking>0) {
	        			...
	        			isAbove50=false;
	        			if(greaterThan50<25) {
	        				System.out.println(greaterThan50+" tickets are booked with a fine of 18% extra on fare charges.");
	        				fare=fare+fare*0.18;
	        				totalFare=totalFare+(fare*greaterThan50);
		        			seatsBooked+=greaterThan50;
		        			totalSeats-=greaterThan50;
	        			}else {
	        				System.out.println("25 tickets are booked with a fine of 18% extra on fare charges.");
	        				fare=fare+fare*0.18;
	        				totalFare+=(fare*25);
	        				totalSeats-=25;
	        				seatsBooked+=25;
	        				int greaterThan75=seatBooking-seatsBooked;
	        				System.out.println(greaterThan75+" tickets are booked with a fine of 32% extra on fare charges.");
	        				fare=264;
        			...
        			if(greaterThan50>seatBooking) {
        				...
        				System.out.println(seatBooking+" tickets are booked with a fine of 18% extra on fare charges.");
        				totalFare+=(fare*seatBooking);
        				totalSeats-=seatBooking;
        				seatsBooked+=seatBooking;
        			}else {
...
//	        		seatBooking=totalSeats;
	        		if(isAbove50) {
	        			...
	        			if(greaterThan50>seatBooking) {
	        				...
	        				totalSeats-=seatBooking;
	        				seatsBooked+=seatBooking;
	        			}else if(totalSeats<25 && seatBooking>totalSeats) {
	        				if(fare==200 || fare==236) {
	        					fare=264;
	        				}
	        				totalFare+=fare*totalSeats;
	        				seatsBooked+=totalSeats;
	        				System.out.println(totalSeats+" tickets are booked with a fine of 32% extra on fare charges.");
	        			}
	        			else {
	        				if(fare==200) {
	        					fare+=fare*0.18;
		        			...
		        			seatsBooked+=greaterThan50;
		        			totalSeats-=greaterThan50;
	        					int greaterThan75=(seatBooking-greaterThan50);
		        				System.out.println(greaterThan75+" tickets are booked with a fine of 32% extra on fare charges.");
		        				fare=264;
		        				totalFare+=fare*greaterThan75;
		        				totalSeats-=greaterThan75;
	...
	double formattedTotalFare = Double.parseDouble(
			formattedValue);System.out.println("Ok...Thanks for your bookings. We have reserved you with "+seatsBooked+" seats. Your total fare is "+formattedTotalFare);if(age>50&&!isFemale)
	{
	...
	pList.add(new Passenger(pId,pName,age,gender,seatsBooked,formattedTotalFare));}System.out.println("\n\nHere is the passenger list:");System.out.println("=======================================================================");for(Passenger p:pList)
	{
